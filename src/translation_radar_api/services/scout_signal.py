...
from translation_radar_api.models import AlertReason, ResearcherRecord, ScoutSignalAlert, ScoutSignalResponse


def score_researcher_signal(researcher: ResearcherRecord) -> ScoutSignalAlert | None:
    ...
    if score >= 0.8:
        ...
        recommendation = "Review with scout lead"
    else:
        recommendation = "Monitor"

    return ScoutSignalAlert(
...
def build_scout_signals(researchers: list[ResearcherRecord], minimum_score: float = 0.55) -> ScoutSignalResponse:
    alerts = []
    for researcher in researchers:
        alert = score_researcher_signal(researcher)
        if alert and alert.alert_score >= minimum_score:
            alerts.append(alert)

    alerts.sort(key=lambda item: item.alert_score, reverse=True)
    return ScoutSignalResponse(alerts=alerts)